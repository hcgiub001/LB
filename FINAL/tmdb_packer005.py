...
This variant *omits offsets by default* to produce a smaller raw .bin. The
header.flags bit0 will be set to 1 to indicate offsets are omitted; readers
must reconstruct offsets from lengths. Double-click (no flags) will omit offsets.

If you later want offsets included, run a separate tool to insert offsets or
modify this script to force args.omit_offsets = False.
"""
...
def main():
    ...
    # FORCE OMISSION of offsets for the default/double-click path to produce a smaller .bin.
    # If you want to write offsets instead, change this line to False.
    args.omit_offsets = True
    ...
    print("Binary format: header + source_keys + (offsets omitted) lengths + remap_table + values_blob")
