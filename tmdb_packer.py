...
(and later for a Python/Kodi reader using mmap).

Usage:
    python3 tmdb_packer.py /path/to/file.ndjson
or
    python3 tmdb_packer.py         # opens a file picker (tkinter) if available

Outputs (default ./packed_output/):
  - dataset.bin
  - dataset.bin.gz
  - dataset.bin.br   (if brotli installed)
  - remap_table.csv
  - source_keys.csv
  - metadata.json
  - analysis_stats.json

Binary header (28 bytes, little-endian):
 offset length contents
 0      4      magic 'SIML' (0x53 49 4D 4C)
 4      1      version (1)
 5      1      endianness (1 = little)
 6      2      flags (reserved = 0)
 8      4      R (uint32)
12      4      E (uint32)
16      4      U (uint32)
20      1      lengths_type (0=uint8,1=uint16)
21      1      remap_index_width (2,3,4)
22      2      reserved
24      4      header_crc32 (uint32)  -- patched after writing file

Layout (immediately after header, in order):
 - source_keys: R * uint32
 - offsets:     R * uint32  (start index into values_blob)
 - lengths:     R * uint8/uint16
 - remap_table: U * uint32  (packed values)
 - values_blob: E * remap_index_width bytes (little-endian per index)
...
def parse_ndjson(path):
    """
    Parse NDJSON file producing:
      - rows: list of dicts {source_tmdb_id, source_type, packed_source_key, packed_list}
      - total_similar E (int)
      - freq Counter mapping packed_value -> count
      - freq_by_type Counter for movie/tv
      - max_list_len (int)
    """
    rows = []
    freq = Counter()
...
def build_remap(freq_counter):
    """
    Create remap sorted by frequency descending (tie-break by packed value).
    Returns:
      - packed_vals: list of packed_value in remap-index order
      - freqs: list of frequencies same order
      - pv_to_idx: dict packed_value -> remap_index
    """
...
def choose_remap_width(U):
    """Choose minimal remap_index_width in bytes for U unique items."""
    if U <= 0xFFFF:
        return 2
    if U <= 0xFFFFFF:
        return 3
...
def write_uint_bytes(fh, value, width):
    """Write value in little-endian using width bytes (2/3/4)."""
    if width == 2:
...
def write_bin_file(out_path, rows_sorted, offsets, header_info, packed_vals, pv_to_idx, remap_width, lengths_type):
    """
    Write the binary file.
    header_info: dict with R,E,U
    rows_sorted: rows sorted by packed_source_key
    offsets: list of start indices
    packed_vals: remap table (index -> packed)
    pv_to_idx: mapping packed->index
    remap_width: 2/3/4 bytes per index in values blob
    lengths_type: 0 (uint8) or 1 (uint16)
    """
    R = header_info['R']; E = header_info['E']; U = header_info['U']
    header_size = 28
    # Compose header with crc 0 placeholder
    magic = b"SIML"
    version = 1
    endian = 1
    flags = 0
    ...
    # Write to temp file then patch CRC
...
def main():
    ...
    print("Now you can implement a Tampermonkey reader that mmap/reads this file using TypedArrays.")
    return
