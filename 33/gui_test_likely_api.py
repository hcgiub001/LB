...
Place alongside likely_api.py (and dataset_core.py if used).
...
def import_likely_api_with_stubs() -> Any:
    try:
        ...
        if not os.path.exists(os.path.join(_THIS_DIR, "likely_api.py")):
            raise RuntimeError(
                "likely_api.py not found in current directory. Place your likely_api.py here before running this tester."
            ) from exc
