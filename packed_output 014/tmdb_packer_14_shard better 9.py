#!/usr/bin/env python3
"""
tmdb_packer_sharded_fast.py — updated to emit per-shard min/max keys and a compact binary shards_index.bin.

Index format (shards_index.bin):
  Header (little-endian):
    4 bytes: magic = b"SIDX"
    1 byte : version = 1
    1 byte : key_width_bytes (4 or 8)
    2 bytes: reserved (0)
    4 bytes: shard_count (uint32)
  Body:
    shard_count entries of min_packed_key, each as uint32 (if key_width_bytes==4) or uint64 (if 8)

Use bisect_right on the loaded mins array and subtract 1 to find candidate shard index.
"""
import os
import sys
import json
...
def write_sharded_bins(outdir, base_name, shards, args):
    ...
    for i, shard_rows in enumerate(shards):
        ...
        # compute per-shard min/max packed source key (contiguous range because rows_sorted)
        packed_keys = [r['packed_source_key'] for r in shard_rows]
        min_key = min(packed_keys) if packed_keys else 0
        max_key = max(packed_keys) if packed_keys else 0

        print(f"Writing shard {i+1}/{len(shards)} -> {os.path.basename(shard_bin)} R={R} E={E} U={U} remap_w={remap_width} lengths={'uint8' if lengths_type==0 else 'uint16'} min_key={min_key} max_key={max_key}")
        ...
        shard_meta = {
            ...
            # new fields for index
            "min_packed_key": int(min_key),
            "max_packed_key": int(max_key)
        }
        manifest.append(shard_meta)
    return manifest

def write_shards_index(outdir, manifest):
    """
    Write a compact binary index containing only the per-shard min_packed_key values (ascending).
    Chooses 4 or 8 byte width depending on max key among shards.
    Returns (path, key_width_bytes, shard_count)
    """
    mins = [int(m['min_packed_key']) for m in manifest]
    if not mins:
        # create empty index
        key_width = 4
    else:
        overall_max = max(int(m['max_packed_key']) for m in manifest)
        # choose smallest width that fits overall_max
        if overall_max <= 0xFFFFFFFF:
            key_width = 4
        else:
            key_width = 8

    idx_path = os.path.join(outdir, 'shards_index.bin')
    magic = b"SIDX"
    version = 1
    reserved = 0
    shard_count = len(mins)
    with open(idx_path, 'wb') as fh:
        # header: magic(4) version(1) key_width(1) reserved(2) shard_count(uint32)
        fh.write(struct.pack('<4s B B H I', magic, version, key_width, reserved, shard_count))
        # body: mins as uint32 or uint64
        if key_width == 4:
            for v in mins:
                fh.write(struct.pack('<I', v & 0xFFFFFFFF))
        else:
            for v in mins:
                fh.write(struct.pack('<Q', v & 0xFFFFFFFFFFFFFFFF))
    return os.path.abspath(idx_path), key_width, shard_count

def write_manifest(outdir, manifest, global_meta, shards_index_path=None, index_key_width=None):
    ...
    if shards_index_path:
        out['shards_index'] = {
            "path": shards_index_path,
            "key_width_bytes": index_key_width
        }
    with open(manifest_path, 'w', encoding='utf-8') as fh:
        json.dump(out, fh, indent=2)
    print("Wrote manifest:", manifest_path)
    if shards_index_path:
        print("Wrote shards index:", shards_index_path)
...
def main():
    ...
    # write compact shards index (mins only) to minimize memory for readers
    shards_index_path, index_key_width, shard_count = write_shards_index(outdir, manifest)
    ...
    global_meta = {
        ...
        "shards_index": os.path.abspath(shards_index_path),
        "shards_index_key_width_bytes": index_key_width
    }
    write_manifest(outdir, manifest, global_meta, shards_index_path=shards_index_path, index_key_width=index_key_width)
